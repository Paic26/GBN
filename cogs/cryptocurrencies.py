...
def get_doge():
    global ERROR_READ
    url = 'https://www.worldcoinindex.com/coin/dogecoin'
    page = urllib3.PoolManager(cert_reqs='CERT_REQUIRED', ca_certs=certifi.where()).request('GET', url)
    soup = BeautifulSoup(page.data, 'html.parser')
    try:
        doge_price = soup.find('div', attrs={'class': 'col-md-6 col-xs-6 coinprice'}).text
        doge_price = re.sub("[^0-9.,$]", "", doge_price)
        print(doge_price)
        doge_change = soup.find('div', attrs={'class': 'col-md-6 col-xs-6 coin-percentage'}).text
        doge_change = re.sub("[^0-9.,%\-+]", "", doge_change)
        print(doge_change)
        return "DOGE: {}, change: {}".format(doge_price, doge_change)
    except AttributeError:
        return ERROR_READ
...
class Crypto(commands.Cog):
    ...
    async def doge(self, ctx, currency='doge', interval: int = 2, hour=datetime.now().hour, input_date=str(date.today())):
        global ERROR_READ
        global currencies
        interval *= 60
        input_date = [int(item) for item in input_date.split('-')]
        ...
        while date.today() <= input_date and datetime.now().hour <= hour:
            try:
                ...
                if not data == ERROR_READ:
                    ...
                    embed.set_author(name="💰 DOGE COIN 💰", icon_url="https://www.telegraph.co.uk/content/dam/technology/2021/01/28/Screenshot-2021-01-28-at-13-20-35_trans_NvBQzQNjv4BqEGKV9LrAqQtLUTT1Z0gJNRFI0o2dlzyIcL3Nvd0Rwgc.png")
                    embed.add_field(name=f"{data}", value="\u200b", inline=False)
